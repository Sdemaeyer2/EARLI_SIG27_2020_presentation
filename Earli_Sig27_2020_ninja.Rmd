...
#load("~/Praatjes/Congressen/EARLI SIG 27 - 2020/M1_Fit")
#load("~/Praatjes/Congressen/EARLI SIG 27 - 2020/M2a_Fit")
#load("~/Praatjes/Congressen/EARLI SIG 27 - 2020/M2b_Fit")
#load("~/Praatjes/Congressen/EARLI SIG 27 - 2020/M3_Fit")
...
$$
\begin{aligned}
y_{i(jk)} = & \theta     \times (\beta_{1} + \nu_{1j} + \eta_{1k} + \epsilon_{1i(jk)}) + \\
...
- $\nu_{1k}$ & $\nu_{1k}$ = unique effect of comparison $k$ on 'scanning' & 'text comprehension reading';
- $\epsilon_{1i(jk)}$ & $\epsilon_{2i(jk)}$ = residuals;
- $\theta$ = mixing proportion (weight)
...
The material is shared on OSF: url(https://osf.io/evrsf/)
